    ...
    const handleExtractUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (data.charges && Array.isArray(data.charges)) {
                // Map API charges to UUID structure
                const newCharges = data.charges.map((c: any) => ({
                    id: crypto.randomUUID(),
                    date: c.date,
                ...
                onChargesChange([...charges, ...newCharges]);
                alert(`Sucesso! ${newCharges.length} cobranças identificadas.`);
            }
        } catch (error) {
            console.error(error);
            alert("Erro ao analisar extrato. Tente novamente.");
        } finally {
        ...
        <div className="space-y-6 animate-fade-in">
                    ...
                    <div className="flex gap-2">
                        <div className="relative">
                            <input
                                ...
                                id="extract-upload"
                                className="hidden"
                                accept="image/*,.pdf" // Note: PDF handling requires extra step, sticking to image for now or handling both if API supports
                ...
                {charges.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {charges.map((charge) => (
                            <div
                                key={charge.id}
                                className="grid grid-cols-12 gap-3 items-end p-3 bg-secondary/30 rounded-lg border border-border"
                            >
            ...
            <div className="bg-primary/5 border border-primary/20 rounded-lg p-5">
                ...
                <div className="space-y-2 text-sm">
                    ...
                    <div className="section-divider border-b border-border/50 my-3" />
