...
async function processImages() {
    ...
    for (const file of files) {
        ...
        try {
            ...
            // 3. Pixel manipulation to remove background and solidify text
            image.scan(0, 0, image.bitmap.width, image.bitmap.height, function (x, y, idx) {
                ...
                // Simple luminance or just check value since it's greyscale (r=g=b)
                // Threshold: If bright > 200, assume background -> Transparent
                if (red > 200) {
            ...
            // 4. Autocrop to remove excess empty space
            image.autocrop({ tolerance: 0.05 }); // slightly loose crop if needed, or strict ? 0 tolerance since we set alpha 0 manually

            // 5. Write
            const outName = file.split('.')[0] + '.png'; // Force PNG for transparency
            await image.writeAsync(path.join(outputDir, outName));
